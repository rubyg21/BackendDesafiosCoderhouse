...
router.post("/:cid/product/:pid", async (req, res) => {
  ...
  let enExistencia = ProductManager.getProducts().some(
    (el) => el.id === Number(pid)
  );
  if (!enExistencia)
    res
      .status(404)
      .json({ error: "El producto no se encuentra en la base de datos" });
  else {
    (await CartManager.addToCart(Number(cid), Number(pid), quantity))
      ? res
          .status(201)
          .json({ Info: `Elementos agregados a carrito numero ${Number(cid)}` })
      : res
          .status(400)
          .json({
            error:
              "Ha ocurrido un error agregado productos, asegurese de proveer el numero de carrito correcto",
          });
  }
});
